...
export class NotificationsService {
  ...
  private async sendEmail(alert: Alert, emailConfig: NonNullable<NotificationConfig['email']>): Promise<NotificationResult> {
    try {
      ...
      const from = emailConfig.from || process.env.EMAIL_FROM || 'alerts@girlycrea.com';
  ...
  private async sendSlack(alert: Alert, slackConfig: NonNullable<NotificationConfig['slack']>): Promise<NotificationResult> {
    try {
      ...
      const payload = {
        ...
        username: slackConfig.username || 'GirlyCrea Alerts',
        icon_emoji: ':warning:',
        attachments: [
          {
            color: severityColor,
...
<body>
  <div class="container">
    ...
    <div class="footer">
      <p>GirlyCrea Monitoring System</p>
    </div>
  </div>
</body>
</html>
...
export function getDefaultNotificationConfig(): NotificationConfig | null {
  ...
  if (process.env.ALERT_EMAIL_TO) {
    ...
    config.email = {
      ...
      from: process.env.ALERT_EMAIL_FROM || process.env.EMAIL_FROM || 'alerts@girlycrea.com',
    };
  }
  ...
  if (process.env.SLACK_WEBHOOK_URL) {
    ...
    config.slack = {
      ...
      username: process.env.SLACK_USERNAME || 'GirlyCrea Alerts',
    };
  }
